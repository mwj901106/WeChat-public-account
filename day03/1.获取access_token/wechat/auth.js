...
(async ()=>{
  ...
  let readResult = await w.readAccessToken()

  if(readResult){
    console.log('本地存在access_token')
    if(w.isValidAccessToken(readResult)){
      console.log('本地存在access_token，并且有效')
      console.log(readResult)
      return readResult
    }else{
      console.log('本地存在access_token，但过期，需要重新获取')
      let newAccessToken = await w.getAccessToken()
      await w.saveAccessToken(newAccessToken)
      console.log(newAccessToken)
      return newAccessToken
    }
  }else{
    let firstData = await w.getAccessToken()
    await w.saveAccessToken(firstData)
